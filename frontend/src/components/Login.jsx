...
const Login = () => {
  ...
  return (
    <div>
        ...
        <form onSubmit={getInputData} className='flex flex-col w-3/12 p-12 my-48 left-0 right-0  mx-auto items-center justify-center absolute  rounded-lg bg-black opacity-90'>
            ...
            <div className='flex flex-col'>
                ...
                <button type='submit' className='bg-red-600 mt-6 p-3 text-white rounded-md font-medium'>{(isLogin?"Login":"Signup")}</button>
                <p className='text-white mt-2 px-4'>{isLogin ? "New to Netflix?" : "Already have an account?"}<span onClick={loginHandler}  className='ml-1 text-blue-900 font-medium cursor-pointer'>{isLogin ? "Signup" : "Login"}</span></p>
            </div>
        </form>
